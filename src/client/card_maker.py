...
class ModifyDialog(wx.Dialog):
    def __init__(self, parent, ID, title, card_id, size=wx.DefaultSize, pos=wx.DefaultPosition, style=wx.DEFAULT_DIALOG_STYLE):
        pre = wx.PreDialog()
        ...
        st = wx.StaticText (self, -1, 'id:')
        gbs.Add (st, (0,0), flag=wx.ALIGN_RIGHT)
        self.st_id = wx.StaticText (self, -1, '')
        gbs.Add (self.st_id, (0,1))
        
        st = wx.StaticText (self, -1, 'Name:')
        gbs.Add (st, (1,0), flag=wx.ALIGN_RIGHT)
        tc = self.card_name = wx.TextCtrl (self, -1, 'test')
        gbs.Add (tc, (1,1))
        
        st = wx.StaticText (self, -1, 'Image:')
        gbs.Add (st, (2,0), flag=wx.ALIGN_RIGHT)
        tc = self.card_image = wx.TextCtrl (self, -1, '')
        gbs.Add (tc, (2,1))
        
        btn = wx.Button (self, -1, "Modify")
        btn.SetDefault()
        gbs.Add (btn, (3,0))
        ...
        gbs.Add (btn, (3,1))
        self.Bind (wx.EVT_BUTTON, self.OnRemove, btn)
        
        sizer.Add (gbs, 0, wx.ALL, 10)
        
        self.SetSizerAndFit(sizer)
        
        self.load_card_info(card_id)
    
    def load_card_info (self, card_id):
        self.card_id = card_id
        card = self.card = lib.lib[card_id]
        self.st_id.SetLabel (str(card_id))
        self.card_name.SetValue (card.name)
        self.card_image.SetValue (card.image)
    
    def OnModify(self, event):
        self.card.name = self.card_name.GetValue()
        self.card.image = self.card_image.GetValue()
        lib.lib[self.card_id] = self.card
        lib.save()
        self.EndModal(wx.ID_OK)
    
    def OnRemove(self, event):
        lib.remove(self.card_id)
        lib.save()
        self.EndModal(wx.ID_OK)
...
#        sizer = wx.GridSizer (1, 1, 0, 0)
        ...
        #nb.SetConstraints(anchors.LayoutAnchors(nb, True,True,True,True))
#        sizer.Add (nb, wx.ALL)
#        self.panel.SetSizerAndFit (sizer)
...
class MyApp(wx.App):
    ...
    def OnInit(self):
        win = MyFrame(None, -1, "OurGathering Card Maker")
        win.Show(True)
